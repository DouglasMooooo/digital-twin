...
async function testABTesting(): Promise<void> {
  ...
  try {
    const testVariants: Record<string, { name: string; score: number }> = {
    ...
    // Determine winner
    let winner = { name: 'Standard STAR', score: 0.8, key: 'control' };
    for (const [key, val] of Object.entries(testVariants)) {
      if (val.score > winner.score) {
        winner = { name: val.name, score: val.score, key };
      }
    }
