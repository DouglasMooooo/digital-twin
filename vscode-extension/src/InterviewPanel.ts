...
export class InterviewPanel {
  ...
  constructor(extensionUri: vscode.Uri, mcpClient: any) {
   ...
   */
  private async updateMetrics(period: 'daily' | 'weekly' | 'monthly' = 'daily'): Promise<void> {
    ...
    try {
      ...
      const reportText = (response.content as any)?.[0]?.text;
   ...
   * Run A/B test
   */
  private async runABTest(questionId: string, question: string): Promise<void> {
    if (!this.panel || !this.mcpClient) {
      return;
    }
    ...
    try {
      ...
      const sessionText = (response.content as any)?.[0]?.text;
   ...
   */
  private async getRecommendations(userId: string = 'default'): Promise<void> {
    if (!this.panel || !this.mcpClient) {
      return;
    }
    ...
    try {
      ...
      const recommendationsText = (response.content as any)?.[0]?.text;
